...
class SmhImporter
  ...
  # This method interprets the description to return the
  # summary minus everything else
  ...
  def scrape
    ...
    open(url) do |rss|
      ...
      feed.items.each do |item|
        # Create Article object for each item
        @source = Source.find_by_name("Sydney Morning Herald")
        summ = interpret_summary(item.description)
        image = interpret_image(item.description)
